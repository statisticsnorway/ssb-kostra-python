...
class TestRoundingAndKonverterDtypes:
    ...
    def test_konverter_dtypes_converts_groups_correctly(self, mocker):
        ...
        assert np.allclose(out["d1"].values, [1.3, 1.4, -1.3, np.nan], equal_nan=True)
        assert np.allclose(
            out["d2"].values, [2.35, 2.36, -2.35, np.nan], equal_nan=True
        )
    ...
    def test_konverter_dtypes_warns_for_missing_and_unknown_group(self, mocker):
