...
# `datasett_reelle_data = hente_data_folkemengde(2024, "bydel")`, `datasett_reelle_data = hente_data_folkemengde(2024, "bydel", False)` og `datasett_reelle_data = hente_data_folkemengde(2024, "bydel", testdata = False)` er likestilt. I dette tilfellet hentes et ordentlig datasett, og ikke et testdatasett. Om du ikke angir at du skal lage testdata **(testdata = True)** eller ganske enkelt **(True)**, genererer funksjonen automatisk et datasett for år **20XX** som er laget av befolkningsdata for det samme året. False betyr altså det motsatte, du ønsker ikke å lage testdata, men heller reelle data.
...
# `testdatasett = hente_data_folkemengde(2024, "bydel", True)`, alternativt `testdatasett = hente_data_folkemengde(2024, "bydel", testdata = True)`, vil generere et datasett for **2024**, men inputdataene vil komme fra en **2023**-fil. Når du setter **testdata = True** eller ganske enkelt **True**, bestemmer du at du ønsker å generere data for **t** med data fra **t-1**.
