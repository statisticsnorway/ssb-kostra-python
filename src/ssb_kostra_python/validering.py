...
SEPARATOR = (
    "-" * 230   # or whatever length you need
)
...
def show_toggle(df, mask, title, *, preview_rows: int = 15):
    ...
    """
    count = int(mask.sum())
    header = widgets.HTML(f"<b>{title}</b> &nbsp; <span style='color:#666'>({count} rows)</span>")
    if count == 0:
...
def _missing_cols(inputfil, klassifikasjonsvariable):
    ...
    if missing_cols:
        ...
        logger.info(f"✅ No missing columns. All {klassifikasjonsvariable} are present.\n")


# %%
def _missing_values(df, klassifikasjonsvariable, preview_rows: int = 10, zeros_valid_for=None):
    """
    ...
    if missing_cols:
        logger.error(f"❌ Columns not found (cannot check for missing): {missing_cols}\n")
    ...
    for col in (c for c in klassifikasjonsvariable if c in df.columns):
        ...
        if mask_missing.any():
            ...
            show_toggle(df, mask_missing, f"Missing values in '{col}' — click to preview", preview_rows=preview_rows)

    if not any_issues and not missing_cols:
        logger.info(f"✅ No missing values in any of the klassifikasjonsvariable {klassifikasjonsvariable}.\n")
...
def _valid_periode_region(df, klassifikasjonsvariable, preview_rows: int = 10):
    ...
    for col in klassifikasjonsvariable:
        ...
        if col == "periode":
            ...
            mask_fmt_bad = ~mask_missing & ~mask_padded_missing & ~s_norm.str.fullmatch(r"\d{4}")

            if mask_padded_missing.any():
                logger.warning(f"⚠️ Suspected zero-padded missing in '{col}'. Defer to missing-values check.\n")
                # display(df.loc[mask_padded_missing].head(preview_rows))
                show_toggle(df, mask_padded_missing, "Padded-missing 'periode' — click to preview", preview_rows=15)

            if mask_fmt_bad.any():
                logger.error(f"❌ Check: {col} is not four digits.\n")
                # display(df.loc[mask_fmt_bad].head(preview_rows))
                show_toggle(df, mask_fmt_bad, "Format-invalid 'periode' — click to preview", preview_rows=15)
        ...
        if col in ["kommuneregion", "fylkesregion"]:
            ...
            mask_numeric = ~mask_missing & ~mask_padded_missing & s_norm.str.fullmatch(r"\d+")
            mask_fmt_bad = mask_numeric & (s_norm.str.len() != 4)

            if mask_padded_missing.any():
                logger.warning(f"⚠️ Suspected zero-padded missing in '{col}'. Defer to missing-values check.\n")
                # display(df.loc[mask_padded_missing].head(preview_rows))
                show_toggle(df, mask_padded_missing, f"Padded-missing '{col}' — click to preview", preview_rows=15)

            if mask_fmt_bad.any():
                logger.error(f"❌ Check: {col} is not four digits.\n")
                # display(df.loc[mask_fmt_bad].head(preview_rows))
                show_toggle(df, mask_fmt_bad, f"Format-invalid '{col}' — click to preview", preview_rows=15)
            elif not mask_padded_missing.any():
        ...
        if col == "bydelsregion":
            ...
            mask_range_ok = mask_numeric & (s_norm.str.len() == 6) & numeric_vals.between(30101, 39999)
            ...
            if mask_padded_missing.any():
                logger.warning(f"⚠️ Suspected zero-padded missing in '{col}'. Defer to missing-values check.\n")
                # display(df.loc[mask_padded_missing].head(preview_rows))
                show_toggle(df, mask_padded_missing, "Padded-missing 'bydelsregion' — click to preview", preview_rows=15)

            if mask_fmt_bad.any():
                logger.error(f"❌ Column '{col}' must be 6-digit numeric in 030101–039999.\n")
                # display(df.loc[mask_fmt_bad].head(preview_rows))
                show_toggle(df, mask_fmt_bad, "Format-invalid 'bydelsregion' — click to preview", preview_rows=15)
            elif not mask_padded_missing.any():
...
def _number_of_periods_in_df(inputfil, preview_rows: int = 10):
    ...
    for v in uniq:
        if pd.isna(v) or (isinstance(v, str) and v.strip() == ""):
            true_missing.append(v); continue
        sv = str(v).strip()
        core = sv.lstrip("0").lower()
        if core in TOKENS:
            padded_missing.append(v); continue
    ...
    if mask_padded.any():
        logger.warning(f"⚠️ Suspected padded-missing periods ({int(mask_padded.sum())} rows) → see results from the missing-values check.\n")
        # display(df.loc[mask_padded].head(preview_rows))
        show_toggle(inputfil, mask_padded, f"Suspected padded-missing periods — click to preview", preview_rows=preview_rows)

    # 2) True missing → error + display rows
    if mask_missing.any():
        logger.error(f"❌ Missing 'periode' values ({int(mask_missing.sum())} rows) → handled by the missing-values check.\n")
        # display(df.loc[mask_missing].head(preview_rows))
        show_toggle(inputfil, mask_missing, f"Missing 'periode' values — click to preview", preview_rows=preview_rows)

    # 3) Format-invalid → error + display rows
    if mask_fmt.any():
        logger.error(f"❌ Format-invalid 'periode' tokens ({int(mask_fmt.sum())} rows) → handled by the format check.\n")
        show_toggle(inputfil, mask_fmt, f"Format-invalid 'periode' tokens — click to preview", preview_rows=preview_rows)
...
def _klass_check(
    ...
    if "periode" in klassifikasjonsvariable and "periode" in df.columns:
        ...
        if antall_perioder == 1:
            ...
            logger.info(f"ℹ️ Checking if all region values are found in KLASS for the year {periode}...\n")
        elif antall_perioder == 0:
            logger.warning("⚠️ No valid 4-digit 'periode' detected; KLASS check skipped.\n")
        else:
            logger.warning(
    ...
    unknown_cols = [c for c in klassifikasjonsvariable if c not in known_keys and c in df.columns]
    ...
    def _prompt_klass_id(colname: str) -> int | None:
        if not interactive:
            logger.warning(f"⚠️ No mapping provided for '{colname}' (non-interactive run). Skipping.\n")
            return None
        tries = 3
        while tries > 0:
            try:
                raw = input(f"Enter KLASS ID for '{colname}' (press Enter to skip): ").strip()
            except (EOFError, KeyboardInterrupt):
                logger.warning(f"⚠️ Input not available; skipping '{colname}'.\n")
                return None
            if raw == "":
                logger.warning(f"⚠️ User skipped '{colname}'; KLASS check disabled for this column.\n")
                return None
            if not raw.isdigit():
                logger.warning("⚠️ Invalid input; please enter a numeric KLASS ID or press Enter to skip.\n")
                tries -= 1
                continue
            klass_id = int(raw)
            ...
            try:
                ...
                codes = test.get_codes(from_date=f"{periode}-01-01", to_date=f"{periode}-12-31")
                ...
                if df_codes is None or len(df_codes) == 0:
                    logger.warning(f"⚠️ KLASS ID {klass_id} returned no codes for {periode}. Try again or press Enter to skip.\n")
                    tries -= 1
                    continue
                logger.info(f"ℹ️ Verified KLASS ID {klass_id} for '{colname}' and {periode}.\n")
                return klass_id
            except Exception as e:
                logger.warning(f"⚠️ KLASS lookup failed for ID {klass_id}: {e}. Try again or press Enter to skip.\n")
                tries -= 1
        logger.warning(f"⚠️ Giving up on '{colname}' after multiple attempts; skipping.\n")
        return None
    ...
    def _get_klass_codes(klass_id: int, year: str) -> list[str]:
        ...
        codes = (
            df_codes[["code"]]
            .assign(code=lambda s: s["code"].astype(str).str.strip())
            ["code"]
            .tolist()
        )
        _cache[key] = codes
    ...
    for col in klassifikasjonsvariable:
        ...
        if col not in klass_ids:
            ...
            logger.info(f"ℹ️ No KLASS mapping for '{col}'; skipping KLASS validation for this column.\n")
            continue
        ...
        try:
            ...
            logger.error(f"❌ KLASS lookup failed for '{col}' (ID={klass_id}) and periode {periode}: {e}\n")
            continue
        ...
        if missing:
            ...
            show_toggle(df, mask_invalid, f"Invalid code(s) for classification '{col}'  — click to preview", preview_rows=preview_rows)
        else:
...
def validering(inputfil, klassifikasjonsvariable : list = []):
    ...
    _missing_values(inputfil, klassifikasjonsvariable, zeros_valid_for=set(klassifikasjonsvariable))
    print("\n" + SEPARATOR)
