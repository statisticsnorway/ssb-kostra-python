...
def mapping_bydeler_oslo(year : str | int = "2015"):
    ...
    klass_bydeler_oslo = klass_bydeler_oslo[~klass_bydeler_oslo["code_1"].isin(['030116', '030117', 'EAB'])]
    klass_bydeler_oslo = klass_bydeler_oslo[["code_1"]].rename(columns={"code_1": "from"})
...
def mapping_fra_kommune_til_landet(year : str | int):
    ...
    komm_fylk_korr = komm_fylk_korr.rename(columns={
        'sourceCode': 'from',
        'targetCode': 'to',
    })
    ...
    komm_KOSTRA_gr = komm_KOSTRA_gr.rename(columns={
        'sourceCode': 'from',
        'targetCode': 'to',
    })
    ...
    nuskoder = nus.get_codes(f"{year}-01-01")
        ### Oppretter en korrespondansetabell for bydelene i Oslo som tilbyr tjenester
    klass_kommuner_landet=nuskoder.pivot_level()
    klass_kommuner_landet = klass_kommuner_landet[~klass_kommuner_landet["code_1"].isin(['9999'])]
    klass_kommuner_landet = klass_kommuner_landet[["code_1"]].rename(columns={"code_1": "from"})
    ...
    nuskoder = nus.get_codes(f"{year}-01-01")
        ### Oppretter en korrespondansetabell for bydelene i Oslo som tilbyr tjenester
    klass_kommuner_u_oslo=nuskoder.pivot_level()
    klass_kommuner_u_oslo = klass_kommuner_u_oslo[~klass_kommuner_u_oslo["code_1"].isin(['0301', '9999'])]
    klass_kommuner_u_oslo = klass_kommuner_u_oslo[["code_1"]].rename(columns={"code_1": "from"})
    klass_kommuner_u_oslo["to"] = "EAKUO"
    # display(klass_kommuner_u_oslo)
    
    mapping_kommuner = pd.concat([komm_fylk_korr, komm_KOSTRA_gr, klass_kommuner_landet, klass_kommuner_u_oslo], ignore_index=True)
...
def mapping_fra_kommune_til_fylkeskommune(year : str | int):
    ...
    komm_fylkeskommune_korr = komm_fylkeskommune_korr[~komm_fylkeskommune_korr["sourceCode"].isin(['9999'])]
    komm_fylkeskommune_korr = komm_fylkeskommune_korr.rename(columns={
        'sourceCode': 'from',
        'targetCode': 'to',
    })
    
    komm_fylkeskommune_korr = komm_fylkeskommune_korr[['from', 'to']]
    komm_fylkeskommune_korr["from"] = komm_fylkeskommune_korr["from"].astype(str).str.zfill(4)
    komm_fylkeskommune_korr["to"] = komm_fylkeskommune_korr["to"].astype(str).str.zfill(4)
...
def mapping_fra_fylkeskommune_til_kostraregion(year : str | int):
    ...
    fylkeskomm_kostraregion = fylkeskomm_kostraregion.rename(columns={
        'sourceCode': 'from',
        'targetCode': 'to',
    })
    ...
    nuskoder = nus.get_codes(f"{year}-01-01")
    ### Oppretter en korrespondansetabell for bydelene i Oslo som tilbyr tjenester
    klass_fylkeskommuner_landet=nuskoder.pivot_level()
    klass_fylkeskommuner_landet = klass_fylkeskommuner_landet[~klass_fylkeskommuner_landet["code_1"].isin(['9900'])]
    klass_fylkeskommuner_landet = klass_fylkeskommuner_landet[["code_1"]].rename(columns={"code_1": "from"})
    klass_fylkeskommuner_landet["to"] = "EAFK"

    nus = KlassClassification(127, language="nb", include_future=True)
    nuskoder = nus.get_codes(f"{year}-01-01")
    ### Oppretter en korrespondansetabell for bydelene i Oslo som tilbyr tjenester
    klass_fylkeskommuner_landet_u_oslo=nuskoder.pivot_level()
    klass_fylkeskommuner_landet_u_oslo = klass_fylkeskommuner_landet_u_oslo[~klass_fylkeskommuner_landet_u_oslo["code_1"].isin(['0300', '9900'])]
    klass_fylkeskommuner_landet_u_oslo = klass_fylkeskommuner_landet_u_oslo[["code_1"]].rename(columns={"code_1": "from"})
    klass_fylkeskommuner_landet_u_oslo["to"] = "EAFKUO"

    fylkeskomm_kostraregion_korr = pd.concat([fylkeskomm_kostraregion, klass_fylkeskommuner_landet, klass_fylkeskommuner_landet_u_oslo], ignore_index=True)
...
def hierarki(inputfil: pd.DataFrame, aggregeringstype: str | None = None) -> pd.DataFrame:
    """
    ...
    # --- Finn regionkolonne (må være nøyaktig én)
    region_cols_present = [c for c in ["kommuneregion", "fylkesregion", "bydelsregion"] if c in inputfil_copy.columns]
    if len(region_cols_present) == 0:
    ...
    if len(region_cols_present) > 1:
        raise ValueError(f"Fant flere regionskolonner {region_cols_present}. Det skal være nøyaktig én.")

    region_col = region_cols_present[0]

    # --- Normaliser regionkolonne (streng + riktig lengde)
    if region_col == "kommuneregion":
        inputfil_copy["kommuneregion"] = inputfil_copy["kommuneregion"].astype(str).str.zfill(4)
    elif region_col == "fylkesregion":
        inputfil_copy["fylkesregion"] = inputfil_copy["fylkesregion"].astype(str).str.zfill(4)
    else:  # bydelsregion
        inputfil_copy["bydelsregion"] = inputfil_copy["bydelsregion"].astype(str).str.zfill(6)

    # --- Bestem (ev. verifiser) aggregeringstype
    if aggregeringstype is None:
        if region_col == "kommuneregion":
            aggregeringstype = "kommune_til_landet"  # default, kan overstyres ved behov
        elif region_col == "fylkesregion":
            aggregeringstype = "fylkeskommune_til_kostraregion"
        else:  # bydelsregion
            aggregeringstype = "bydeler_til_EAB"
    else:
        # Konsistenssjekk mellom oppgitt aggregeringstype og regionkolonne
        valid_by_region = {
    ...
    # --- Velg mapping, koblingskolonne og etterbehandling
    if aggregeringstype == "kommune_til_landet":
        
        mappingfil = mapping_fra_kommune_til_landet(periode)
        ...
        mappingfil = mapping_fra_fylkeskommune_til_kostraregion(periode)
        join_col = "fylkesregion"
        replace_col = "fylkesregion"
        post_filter = None
        rename_cols = {}

    elif aggregeringstype == "bydeler_til_EAB":
        
        mappingfil = mapping_bydeler_oslo(periode)
        join_col = "bydelsregion"
        replace_col = "bydelsregion"
        post_filter = None
        rename_cols = {}
    ...
    # --- Slå sammen mot mapping
    df_merged = inputfil_copy.merge(mappingfil, left_on=join_col, right_on="from", how="inner")

    # --- Hent klassifikasjons- og statistikkvariable (bruk kopi for å unngå sideeffekter)
    # klassifikasjonsvariable, statistikkvariable = definere_klassifikasjonsvariable(inputfil_copy)
    klassifikasjonsvariable, statistikkvariable = hjelpefunksjoner.definere_klassifikasjonsvariable(inputfil_copy)

    # --- Bytt ut regionverdi med aggregert nivå (to)
    df_merged[replace_col] = df_merged["to"]

    # --- Aggreger
    df_agg = df_merged.groupby(klassifikasjonsvariable, as_index=False, observed=True)[statistikkvariable].sum()

    # --- Kombiner med original
    df_combined = pd.concat([inputfil_copy, df_agg], ignore_index=True)

    # --- Datatyper for klassifikasjonsvariable
    df_combined[klassifikasjonsvariable] = df_combined[klassifikasjonsvariable].astype(str)

    # --- Filtrer (om definert) før ev. kolonnerename (matcher din opprinnelige flyt)
    if post_filter:
        df_combined = post_filter(df_combined)

    # --- Kolonnerename (om definert)
    if rename_cols:
        df_combined = df_combined.rename(columns=rename_cols)
...
def overfore_data_fra_fk_til_k(inputfil):
    ...
    df_merged = mappingfil.merge(
    inputfil,
    left_on="from",
    right_on="fylkesregion",
    how="left"
    )
    ...
    df_merged = (
    df_merged
    .rename(columns={"to": "kommuneregion"})
    .drop(columns=["fylkesregion", "from"])
)
    logger.info("ℹ️ Funksjonen identifiserer nå 'kommuneregion' og ikke 'fylkesregion' som regionsvariabelen i dette datasettet. Dette er ikke en feil.\n" 
...
def gjennomsnitt_aggregerte_regioner(
    ...
    * Du KAN OGSÅ FORHÅNDSDEFINERE de øvrige klassifikasjonsvariablene for å slippe tekstpromptet. Da gjør du som under. Du lager et objekt som du for eksempel kan kalle predefined_input
    ...
    # 2) Perform operation (+ optional rounding), record post-op dtype,
    #    then optionally restore to original dtype.
    for c in cols:
        ...
        # Record dtype immediately after the math (pre-restore)
        ...
        if restore_original_dtype:
            ...
            if is_integer_dtype(orig):
                # Ensure integers before casting back
                rounded = result.round(0)
                if rounded.isna().any():
                    df[c] = rounded.astype(_nullable_int_for(orig))
                else:
                    df[c] = rounded.astype(orig)

            elif is_float_dtype(orig):
                df[c] = result.astype(orig)

            elif is_bool_dtype(orig):
                # Nonzero -> True, zero/NaN -> False/NA (nullable boolean if possible)
                try:
                    df[c] = (result != 0).astype('boolean')
                except TypeError:
                    df[c] = (result != 0).astype(bool)

            else:
                # object/category/etc.: keep numeric result unless you define a policy
                df[c] = result
        else:
            df[c] = result
    ...
    if print_types:
        ...
        # Differences
        post_changes = {c: (original[c], post_op[c]) for c in cols if original[c] != post_op[c]}
        final_changes = {c: (original[c], final[c]) for c in cols if original[c] != final[c]}
